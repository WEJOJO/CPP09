...
int BitcoinExchange::ValidationCheck(std::string &line_, std::queue<std::string> &_strs)
{ /// 지운 후 저장을 위해 참조형 사용
	...
	if (string_s.size()!=2 || string_s.front()[datetime_len-1] != ' ' || string_s.back()[0] != ' ')
	{
    ...
    if (datetime_str.size()!=3 || (string_s.back().size() == 1 && dot_cnt == 1)) // if (datetime_str.size()!=3) ++>> "."만 오는 경우
    {
    ...
    // 각 각의 유효성 확인
    if (this->IsValidDate(t1,t2,t3)==false) //혹은 || datetime_str.size()!=3)
	{
...
void BitcoinExchange::MakeChange(char *_argv)
{
    ...
    if (this->myfile.is_open())
    {
        ...
        while (getline(this->myfile, line)) { //date | value
            ...
            // if (this->ConvertDouble(_val)!=_strs.back() && _val!=floor(_val)) /// | 1-2
            // {
            ...
            {
                std::cout << _strs.front() << " => " << _strs.back() << " = " <<  (it->second) * _val << std::endl; ///_ret이랑 곱하면 안될 것 같음.
            }
            else
            {
